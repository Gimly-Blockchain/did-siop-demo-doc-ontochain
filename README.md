...
```bash
cd <workspace>
git clone git@github.com:Gimly-Blockchain/did-siop-auth-web-demo.git
cd onto-demo-server
```
...
```bash
cd <workspace>/did-siop-auth-web-demo
...
```bash
cd <workspace>/did-siop-auth-web-demo
docker build -t did-siop-auth-web-demo .
docker run -it -p 5001:5001 -p 3000:3000 did-siop-auth-web-demo
```
...
If you are running the web app inside a Windows Subsystem for Linux 2 (WSL2) and run into network problems,
like localhost IP not getting correctly mapped, the simplest workaround is to
download [ngrok](https://ngrok.com/) and start it passing `http 5001` as parameter. After that you will need
to update the `REDIRECT_URL_BASE` environment variable with the domain name and port generated by ngrok and everything
should work fine.

Reference: [did-siop-auth-web-demo](https://github.com/Gimly-Blockchain/did-siop-auth-web-demo)
...
```bash
cd <workspace>
git clone git@github.com:Gimly-Blockchain/rn-did-siop-example-app.git
```
...
Reference: [GimlyID-QR-Code-Generator](https://github.com/Gimly-Blockchain/gimlyid-qr-code-generator)
...
Reference: [RN-GimlyID-QR-Code-Scanner](https://github.com/Gimly-Blockchain/rn-gimlyid-qr-code-scanner):
